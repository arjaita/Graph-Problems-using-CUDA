...
void STCONGraph(int argc, char** argv);
...
//	}
	...
	bool *h_graph_mask = (bool*) malloc(sizeof(bool)*no_of_nodes);			//Fa
	bool *h_updating_graph_mask = (bool*) malloc(sizeof(bool)*no_of_nodes);		//Fva
	bool *h_Red_graph_visited = (bool*) malloc(sizeof(bool)*no_of_nodes);		//Ra  (Appended)
	bool *h_Green_graph_visited = (bool*) malloc(sizeof(bool)*no_of_nodes);		//Ga  (Appended)
	bool *h_Red_updating_graph_visited = (bool*) malloc(sizeof(bool)*no_of_nodes);	//Rva  (Appended)
	bool *h_Green_updating_graph_visited = (bool*) malloc(sizeof(bool)*no_of_nodes);	//Gva  (Appended)
	...
	for( unsigned int i = 0; i < no_of_nodes; i++) 
	{
		...
		h_Red_updating_graph_visited[i]=false;		//Rva=false (Appended)
...
//	source=0;
	...
	{
		fscanf(fp,"%d",&id);
		fscanf(fp,"%d",&cost);	//needed because of the text format of the file
		h_graph_edges[i] = id;
	}
	...
	cudaMemcpy( d_Red_graph_visited, h_Red_graph_visited, sizeof(bool)*no_of_nodes, cudaMemcpyHostToDevice) ;

	//Copy the Visited nodes array to device memory	(Appended)
	bool* d_Green_graph_visited;
	cudaMalloc( (void**) &d_Green_graph_visited, sizeof(bool)*no_of_nodes) ;
	cudaMemcpy( d_Green_graph_visited, h_Green_graph_visited, sizeof(bool)*no_of_nodes, cudaMemcpyHostToDevice) ;

	//Copy the Visited nodes array to device memory	(Appended)
	bool* d_Red_updating_graph_visited;
	cudaMalloc( (void**) &d_Red_updating_graph_visited, sizeof(bool)*no_of_nodes) ;
	cudaMemcpy( d_Red_updating_graph_visited, h_Red_updating_graph_visited, sizeof(bool)*no_of_nodes, cudaMemcpyHostToDevice) ;

	//Copy the Visited nodes array to device memory	(Appended)
	bool* d_Green_updating_graph_visited;
	cudaMalloc( (void**) &d_Green_updating_graph_visited, sizeof(bool)*no_of_nodes) ;
	cudaMemcpy( d_Green_updating_graph_visited, h_Green_updating_graph_visited, sizeof(bool)*no_of_nodes, cudaMemcpyHostToDevice) ;

//	//copy Red frontier length (Appended)
//	int* d_R_length;
//	cudaMalloc( (void**) &d_R_length, sizeof(int)) ;
//	cudaMemcpy( &d_R_length, &h_R_length, sizeof(int), cudaMemcpyHostToDevice) ;
//
//	//copy Green frontier length (Appended)
//	int* d_G_length;
//	cudaMalloc( (void**) &d_G_length, sizeof(int)) ;
//	cudaMemcpy( &d_G_length, &h_G_length, sizeof(int), cudaMemcpyHostToDevice) ;
...
//	int len1,len2;
//	cudaMemcpyFromSymbol(&len1, &rf, sizeof(int),0,cudaMemcpyDeviceToHost);
//	cudaMemcpyFromSymbol(&len2, &gf, sizeof(int),0,cudaMemcpyDeviceToHost);
//
//	printf("length = %d",len1+len2);
//	printf("\nlength = %d",len1);
//	printf("\nlength == %d",len2);
	int *d_len,len;
